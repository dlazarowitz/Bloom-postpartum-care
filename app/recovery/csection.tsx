...
import { csectionRecoveryGuide } from '../../src/data/recovery-csection';

type IoniconsName = React.ComponentProps<typeof Ionicons>['name'];

export default function CsectionRecoveryScreen() {
  const guide = csectionRecoveryGuide;
  const [expandedMilestone, setExpandedMilestone] = useState<string | null>(guide.milestones[0]?.id ?? null);

  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        {/* Important notice for c-section */}
        <View style={styles.surgeryNotice}>
          <Ionicons name="information-circle" size={22} color={colors.secondary} />
          <Text style={styles.surgeryNoticeText}>
            A cesarean section is major abdominal surgery. Please be extra patient and gentle with yourself during recovery.
          </Text>
        </View>
                  ...
                  {milestone.tips.map((tip) => (
                    <View key={tip.id} style={styles.tipCard}>
                      <View style={styles.tipHeader}>
                        <Ionicons name={(tip.icon || 'information-circle') as IoniconsName} size={20} color={colors.secondary} />
...
const styles = StyleSheet.create({
  ...
  surgeryNoticeText: { fontSize: fontSize.md, color: colors.text, flex: 1, lineHeight: 22, fontWeight: fontWeight.medium },
  overviewCard: {
    backgroundColor: colors.secondaryBg,
    borderRadius: borderRadius.lg,
    padding: spacing.lg,
